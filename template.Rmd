...
clean_lyrics_text <- function(full_text) {
  if (!is.na(full_text) && nchar(full_text) > 0) {
    ...
    # Remove the first line if it contains the song title and "Lyrics"
    if (length(lines) > 0 && str_detect(lines[1], "\\s*Lyrics$")) {
      lines <- lines[-1]  # Remove the first line
    }
    
    # Additional clean-up
    lines <- lines[!str_detect(lines, "^\\d+ Contributors") & !str_detect(lines, "Lyrics$")]
