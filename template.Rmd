...
=======
## lyrics and songs 
>>>>>>> 767ddc3336592c381d154dcdcb6d581cd23be003

```{r student3}
library(tidyverse)
#Data for all the songs: includes title, artist, song length, word count, url for lyrics
songs_data <- tibble(
  Title = c(
...
songs_data <- songs_data %>% mutate(row_id = row_number())
 ...
 # Pull lyrics (function from chatgpt)
...
```
Looking at the graph above, we can see little trend in the average title usuage in country songs across the decades sinces 1960. Overall the repetition of titles has increased in 2020 compared to 1960. There are some dips and variation in the trend which may be due to individual songs that do not fit the model. Around 2000, there is a steep increase in title repetition
...
# Step 2: Compute average phrase repetition per decade
