...
get_lyrics_genius <- function(url) {
  tryCatch({
    h <- read_html(url)
    lyrics <- html_nodes(h, ".Lyrics__Container-sc-1ynbvzw-6") %>% html_text2()
...
songs_df_with_lyrics <- songs_data %>%
  ...
  mutate(
    ...
    word_count = if (!is.na(words)) length(words) else NA
  ) %>%
  ungroup()

print(songs_df_with_lyrics) #Currently not working 
